...
ACCOUNT_IMAGE_DIR = IMAGE_DIR / "accounts"
ACCOUNT_ICON_DIR = ACCOUNT_IMAGE_DIR / "icons"
ACCOUNT_PRODUCT_IMAGE_DIR = ACCOUNT_IMAGE_DIR / "products"
...
ACCOUNT_IMAGE_DIR.mkdir(parents=True, exist_ok=True)
ACCOUNT_ICON_DIR.mkdir(parents=True, exist_ok=True)
ACCOUNT_PRODUCT_IMAGE_DIR.mkdir(parents=True, exist_ok=True)
...
def load_json_list(path):
    ...
    if not isinstance(data, list):
        raise ValueError("%s の内容が配列ではありません" % path.name)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/account/load":
            try:
                games = load_json_list(DATA_DIR / "account_games.json")
                accounts = load_json_list(DATA_DIR / "accounts.json")
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/account/save":
                ...
                if not isinstance(games, list):
                    raise ValueError("account gamesデータが不正です")

                if not isinstance(accounts, list):
                    raise ValueError("accountsデータが不正です")

                atomic_write_json(DATA_DIR / "account_games.json", games)
                atomic_write_json(DATA_DIR / "accounts.json", accounts)
